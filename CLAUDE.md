...
**Files are the product (SPEC §3):** `filesDir/harness/` is the single source of truth — PROMPT.md (operating rules, system-prompt head — D-034), SOUL.md, USER.md, MEMORY.md index, memories/, notes/, journal/, chats/ (JSONL), widgets/ (JSON specs), plus harness-v2 state (SUMMARY.md, activity.tsv — D-020; CURSOR.md no longer injected or rewritten — D-034). `HarnessStore` owns all file I/O (atomic temp+rename writes); `MemoryStore` owns memories + recall (keyword overlap v1). Anything derived (indexes, widget refresh caches) must be rebuildable from files. Both stores expose an `onMutation` hook that slides the 30-min debounced backup.

**One agent turn:** `ChatViewModel.send` → system prompt assembled by `HarnessStore.systemPrompt()` (**PROMPT.md head** + soul + user + summary + memory index + recalled + today's note + journal tail + time facts — no cursor, no forced continuity) → `AgentRuntime.runTurn` streams provider events, executes tools through `PermissionGate` → gate either allows instantly or **suspends on a `CompletableDeferred`** while the approval sheet renders (`gate.pending` StateFlow) → tool results ride back in USER-role messages as `tool_result` parts. `AgentTurnService` (FGS `specialUse`) brackets the turn so it survives app switching. 20-iteration cap per turn.
...
**Toolbox (the Android trick, IMPLEMENTATION §5):** busybox/curl/jq ship as fake native libs `jniLibs/<abi>/lib*.so` with `useLegacyPackaging = true` so they land executable in `nativeLibraryDir`; `filesDir/bin/` symlinks dispatch by argv[0] (busybox reads `basename(argv[0])`, and `ProcessBuilder` forces argv[0] = the exec'd path — so the symlink is the only way in). `nativeLibraryDir` moves on every install, so `ToolboxRunner` relinks at every process bootstrap and never trusts persisted paths. Which shell runs — and which bundled binary runs at all — is **probed on device** (D-036, D-037), because Android's seccomp filter SIGSYS-kills non-whitelisted syscalls **per binary** (on Android 14: curl lives, busybox and jq die, exit 159). Ladder: busybox ash → `/system/bin/sh` + busybox on PATH → `/system/bin/sh` + toybox; winner exported as `$AISOUL_TOOLBOX`, and `run_command`'s description is generated from the probe. curl needs two Android fixes: CA bundle from `TrustManagerFactory` (not the `/system/etc/security/cacerts` scan — empty on 14) and DoH for DNS (no `/etc/resolv.conf` exists).
