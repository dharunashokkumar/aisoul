...
## 2026-07-18 — Navigation restructure: persistent bottom bar

**D-038 — Persistent bottom bar with four tabs; everything else is a drill-in.** The app shell (`AiSoulNav`) renders a persistent `BottomBar` on exactly four primary destinations — home (dashboard), chat, files, settings — per DESIGN.md §3's "max 4 bottom-nav items" law. Memory, terminal, history, backup, licenses, subdirectory file views and per-chat views are drill-ins: full-screen routes with a back affordance, no bar. Memory stays one tap away (home widgets → memory, settings → memory) and is also just `memories/` inside the files tab.
*Why:* the prior structure had no stable navigation — every screen hand-rolled its own back button and scattered ghost-button rows, so moving between chat/files/settings took too many taps and had no consistent placement. User asked to keep the good colors but fix positioning, placement, and structure. A first bottom-bar attempt (external) had five tabs, back buttons rendered on the *right* edge (they were passed through `TopBar`'s trailing actions slot), 40sp display titles crammed into the bar in place of each screen's in-content hero, and double horizontal padding on settings/backup — this entry supersedes that draft.
*Structure:* shared `TopBar` (`ui/common/AppShell.kt`) is a quiet utility row — back always on the **left**, an `overline` section label, ghost-button actions on the right; every screen's hero (display/headline/data-hero) stays in its scrolling content per DESIGN.md §3. `Destination` enum defines the four shell routes; hand-drawn `Chat`/`Files`/`Settings` icons added to `Icons.kt`. Bottom bar carries no accent (active = text-primary, inactive = text-tertiary) so the accent budget stays with screen content. Duplicated affordances removed (dashboard's "files"/"settings" ghost buttons → tabs).
*Insets:* the shell consumes `WindowInsets.navigationBars` for tab content while the bar is visible (kills the dead strip screens' own `navigationBarsPadding` would add), and the bar hides while the IME is up so chat/memory inputs sit directly on the keyboard.
*Verify:* `./gradlew :app:assembleDebug` + `:app:testDebugUnitTest` green.
