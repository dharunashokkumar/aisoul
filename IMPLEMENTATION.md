...
- Per-process bootstrap: create `filesDir/bin/` full of **symlinks** (`ls`, `grep`, `sh`, `busybox`, … → `libbusybox.so`; `curl` → `libcurl_exe.so`). Busybox dispatches by `basename(argv[0])`, and `ProcessBuilder` always sets `argv[0]` to the exec'd path — so an applet-named symlink is the *only* way to invoke it (exec'ing `libbusybox.so` directly yields `applet not found`). Relinked every process start, never trusted from a marker file, because `nativeLibraryDir` moves on update.
- **Everything is probed, not assumed (D-036, D-037).** Android's seccomp filter kills non-whitelisted syscalls with SIGSYS (exit 159) **per binary** — on Android 14 curl runs and busybox/jq do not. So bootstrap probes the shell ladder (busybox ash → `/system/bin/sh` + busybox on `PATH` → `/system/bin/sh` + toybox) with `echo p1 && ls / >/dev/null 2>&1 && echo p2`, then probes each bundled binary with `--version`, recording `PATH` / `FUNCTION` / `DEAD`. Dead tools get their symlink deleted so `command -v` doesn't lie; busybox applet links live in `bin/busybox/` and stay off `PATH` unless busybox works, so a dead `ls` never shadows Android's. Winner is exported as `$AISOUL_TOOLBOX`, and `run_command`'s description is generated from the result.
- **curl needs two Android-specific fixes (D-037), both verified against the shipped binary:** the CA bundle comes from `TrustManagerFactory` (Android 14 moved the roots into the conscrypt APEX, so the old `/system/etc/security/cacerts` scan wrote an empty bundle → `http_code=000` on every request), and hostnames resolve over **DoH** via `$CURL_HOME/.curlrc` (Android has no `/etc/resolv.conf`, so a static curl otherwise cannot resolve anything).
...
- Exec environment: `cwd=/harness/workspace`, `HOME` likewise, `PATH=filesDir/bin:/system/bin:/system/xbin` (busybox modes) or `/system/bin:/system/xbin` (toybox mode), minimal env vars, `TMPDIR` inside sandbox. stdout/stderr capped (64 KB, tail-truncated with a marker), default timeout 30 s, `Process.destroyForcibly()` on cancel/timeout.
